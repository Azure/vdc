...
class PowershellScriptExecution(object):
    ...
    def execute(
        ...
        try:
            ...
            if sys.platform == "win32":
                command = \
                        "powershell -NonInteractive -NoLogo -ExecutionPolicy Unrestricted -File {}".format(
                            command)
            else:
                command = \
                        "pwsh -NonInteractive -NoLogo -ExecutionPolicy Unrestricted -File {}".format(
                            command)
            # Package that splits based on whitespace and
            # preserves quoted strings
            import shlex
            command_exec = shlex.split(command)

            # Run the check command.
            result = sp.check_output(
                command_exec, 
                shell=self._sp_shell_flag, 
                universal_newlines=True)
            
            self._logger.debug(
                'The command executed successfuly, producing the following output: {}'
                        .format(result))
